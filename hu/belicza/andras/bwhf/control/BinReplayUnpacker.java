...
public class BinReplayUnpacker {
    ...
    private static final int HEADER_SIZE = 0x279;
	...
	public BinReplayUnpacker( final File replayFile ) throws Exception {
		if ( !replayFile.exists() || replayFile.isDirectory() || replayFile.length() < HEADER_SIZE + 8 ) // Not enough data for id, header and commands length
	...
	public synchronized byte[] unpackSection( final int size ) throws Exception {
		...
		for ( n = 0, m1C = 0; n < count; n++, m1C += buffer.length, m20 += len ) {
			...
			if ( length > size - m20 )
				throw new Exception("length > size - m20");
			...
			if ( len == 0 || len > size )
				throw new Exception("len == 0 || len > size");
